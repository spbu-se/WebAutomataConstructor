    ...
    private createMatrix(): void {
        ...
        for (let i = 0; i < this.edges.length; i++) {
            ...
            for (let j = 0; j < this.edges[i].localValue.length; j++) {
                ...
                let numOfArcs = this.edges[i].localValue[j].numberOfArcs
        ...
        this.alphabet.forEach((value, key) => console.log(`value ' ' key ${value}, ' ', ${key}`))
        this.statements.forEach(value => console.log(`value ${value}`))
