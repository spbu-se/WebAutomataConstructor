...
test("{ q0 -> q1 -> (q2); A = {0, 1}}: 2 steps for 2 edges without loop", () => {
    ...
    expect(result.node.id === dfa.run().node.id && result.counter === dfa.run().counter)
        .toBe(true)
})
...
let testFunc = (dfa: DFA, current: NodeCore) : Step => {
...
test("step by step: { q0 -> q1 -> (q2); A = {0, 1}}: 2 steps for 2 edges without loop", () => {
    ...
    let result = testFunc(dfa, {id: 1, isAdmit: false})
